...
def scanline_search(vertices, M, N):
    ...
    for k in range(K):
        x1, y1 = vertices[k]
        x2, y2 = vertices[(k + 1) % K] #This matches v1 to v2, ..., v3 to v1 by circling using mod
        #We find xkmin etc
...
def f_shading(img, vertices, vcolors):
    ...
    paint_color = flat_color * 255

    for x, y in scanline_search(vertices, M, N):
        if x >= 0 and y >= 0:
            updated_img[y, x] = paint_color
...
def g_shading(img, vertices, vcolors):
    ...
    for x, y in scanline_search(vertices, M, N):
        ...
        # Since this is in the range [0,1] we need to multiply it by 255
...
def t_shading(img, vertices, uv, textImg):
    ...
    for y in range(global_ymin, global_ymax + 1):
        ...
        if len(active_edges) >= 2:
            ...
            # dim=2 corresponds to the Y-axis in vector_interp function
            ...
            for x in range(start_x, end_x + 1):
                # dim=1 corresponds to the X-axis in vector_interp function
